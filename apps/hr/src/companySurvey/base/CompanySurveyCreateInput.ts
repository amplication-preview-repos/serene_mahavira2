/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput } from "./CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput } from "./UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput";

@InputType()
class CompanySurveyCreateInput {
  @ApiProperty({
    required: false,
    type: () => CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput,
  })
  @ValidateNested()
  @Type(() => CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput)
  @IsOptional()
  @Field(() => CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput, {
    nullable: true,
  })
  companySurveyFields?: CompanySurveyFieldCreateNestedManyWithoutCompanySurveysInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput,
  })
  @ValidateNested()
  @Type(() => UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput)
  @IsOptional()
  @Field(() => UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput, {
    nullable: true,
  })
  userSurveyResponse?: UserSurveyResponseCreateNestedManyWithoutCompanySurveysInput;
}

export { CompanySurveyCreateInput as CompanySurveyCreateInput };
