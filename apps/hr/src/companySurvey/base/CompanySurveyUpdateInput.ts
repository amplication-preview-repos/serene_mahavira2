/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CompanySurveyFieldUpdateManyWithoutCompanySurveysInput } from "./CompanySurveyFieldUpdateManyWithoutCompanySurveysInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { UserSurveyResponseUpdateManyWithoutCompanySurveysInput } from "./UserSurveyResponseUpdateManyWithoutCompanySurveysInput";

@InputType()
class CompanySurveyUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CompanySurveyFieldUpdateManyWithoutCompanySurveysInput,
  })
  @ValidateNested()
  @Type(() => CompanySurveyFieldUpdateManyWithoutCompanySurveysInput)
  @IsOptional()
  @Field(() => CompanySurveyFieldUpdateManyWithoutCompanySurveysInput, {
    nullable: true,
  })
  companySurveyFields?: CompanySurveyFieldUpdateManyWithoutCompanySurveysInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => UserSurveyResponseUpdateManyWithoutCompanySurveysInput,
  })
  @ValidateNested()
  @Type(() => UserSurveyResponseUpdateManyWithoutCompanySurveysInput)
  @IsOptional()
  @Field(() => UserSurveyResponseUpdateManyWithoutCompanySurveysInput, {
    nullable: true,
  })
  userSurveyResponse?: UserSurveyResponseUpdateManyWithoutCompanySurveysInput;
}

export { CompanySurveyUpdateInput as CompanySurveyUpdateInput };
